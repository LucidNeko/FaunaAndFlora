...
void main() {
	...
	// DIFFUSE
		...
		vec3 diffuse = dcont * gl_FrontMaterial.diffuse.xyz * gl_LightSource[1].diffuse.xyz;
		//diffuse = vec3(0,0,0);
		...
		if (dotSurfAndDirec < gl_LightSource[0].spotCutoff){ 
					...
					diffuse += dcont * gl_FrontMaterial.diffuse.xyz * gl_LightSource[0].diffuse.xyz;
		}
	...
	// SPECULAR
		...
		if (dotSurfAndDirec < gl_LightSource[0].spotCutoff){ 
					...
					reflection = reflect(-surf2light, norm);
					scont = pow( max( dot(surf2view, reflection), 0.0), gl_FrontMaterial.shininess);
					specular += scont * gl_LightSource[0].specular.xyz * gl_FrontMaterial.specular.xyz * (1-((dotSurfAndDirec - gl_LightSource[0].spotCutoff) / 2));
		}		
		// DIRECTION
		surf2light = normalize(gl_LightSource[2].position.xyz  - vPosition);
