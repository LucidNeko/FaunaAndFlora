...
public:
	...
	void create() {
		...
		for(uint i = 1; i < m_obj->m_points.size(); i++) {
			...
			m_particles[idx]   = m_obj->m_points[i].x;
			m_particles[idx+1] = m_obj->m_points[i].y;
			m_particles[idx+2] = m_obj->m_points[i].z;

			m_particles[idx+3] = m_obj->m_points[i].x;
			m_particles[idx+4] = m_obj->m_points[i].y;
			m_particles[idx+5] = m_obj->m_points[i].z;
		...
		for(uint i = 0; i < m_obj->m_triangles.size(); i++) {
			...
			// m_constraints.push_back(new ParticleConstraintDistance(v1.p, v2.p, 0.1f));
			// m_constraints.push_back(new ParticleConstraintDistance(v2.p, v3.p, 0.1f));
			// m_constraints.push_back(new ParticleConstraintDistance(v3.p, v1.p, 0.1f));
		...
		m_constraints.push_back(new ParticleConstraintPin(p));
